    ...
    if (err) {
      ...
      if (!authResult) {
        ...
        const query = "WITH groupList AS ( (SELECT groupid, groupname, groupdesc, tagname FROM group_) EXCEPT "
            ...
            + "leaderInfoNonZero AS ( "
            + "  SELECT DISTINCT leader, first, last, groupid, count(*) as cubvotes "
            + "FROM group_ NATURAL JOIN groupList JOIN users on group_.leader = users.email JOIN post ON post.postowner = group_.leader JOIN cubvoted ON post.postid = cubvoted.postid GROUP BY post.postowner, group_.leader, users.first, users.last, group_.groupid "
            + "), "
            + "leaderInfoZero AS ( "
            + "  SELECT DISTINCT leader, first, last, groupid, 0 as cubvotes "
  ...
  client.query(query, values, (err, response) => {
    if (err) {
      ...
      if (!authResult) {
        ...
        deleteGroup(req, res, req.body.groupID, req.body.leader);
      }
    }
  });
});
